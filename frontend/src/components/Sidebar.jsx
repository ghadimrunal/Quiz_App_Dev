...
const Sidebar = () => {
  ...
  const handleAnswerSelect = (answerIndex) => {
    const updated = { ...userAnswers, [currentQuestion]: answerIndex };
  ...
  const calculateScore = () => {
    ...
    q.forEach((question, i) => {
      if (userAnswers[i] === question.correctAnswer) correct++;
    });
    return {
      correct,
      total: q.length,
      percentage: q.length ? Math.round((correct / q.length) * 100) : 0,
    };
  };
  ...
  const submitResult = async () => {
    ...
    try {
      ...
      toast.error("Could not save result. Check console.");
    }
  };
  ...
  return (
    <div className={sidebarStyles.pageContainer}>
      ...
      <div
        className={`${sidebarStyles.mainContainer} flex w-full`}
        style={{ alignItems: "flex-start" }}
      >
        {/* SIDEBAR */}
        <aside
          ref={asideRef}
          className={`${sidebarStyles.sidebar} relative`}
          style={{
            background:
              "linear-gradient(180deg, rgba(232,244,255,1) 0%, rgba(250,251,255,1) 40%, rgba(255,255,255,1) 100%)",
            boxShadow:
              "0 4px 12px rgba(22, 38, 63, 0.06), inset 0 -1px 0 rgba(255,255,255,0.6)",
            minWidth: 280,
            maxWidth: 320,
          }}
        >
          {/* HEADER  src/components/Sidebar.jsx  (REPLACE ONLY THIS BLOCK)*/}
                  ...
                  <div className={sidebarStyles.levelsContainer}>
                    <h3 className={sidebarStyles.levelsTitle}>
                      ...
                      <span className={sidebarStyles.techBadge}>
                        {technologies.find((x) => x.id === selectedTech).name}
                      </span>
                    </h3>
                    ...
                    {levels.map((level) => (
                      ...
                      >
                        ...
                        <span
                          className={sidebarStyles.levelQuestions}
                          style={{ color: "#0f172a", fontWeight: 500 }}
                        >
                          {level.name} • {level.questions} Qs
                        </span>
                      </button>
                    ))}
                  </div>
        ...
        >
          {/* MOBILE HEADER */}
          <div
            className={sidebarStyles.mobileHeader}
                  ...
                  <div className={sidebarStyles.resultsHeader}>
                    ...
                    <div className={sidebarStyles.scoreGrid}>
                      <div className={sidebarStyles.scoreCard}>
                        <div className="inline-flex items-center justify-center w-12 h-12 bg-emerald-100 text-emerald-600 rounded-full mb-3 shadow-inner">
                          <CheckCircle size={24} />
                        </div>
                      ...
                      <div className={sidebarStyles.scoreCard}>
                        <div className="inline-flex items-center justify-center w-12 h-12 bg-rose-100 text-rose-600 rounded-full mb-3 shadow-inner">
                          <XCircle size={24} />
                        </div>
